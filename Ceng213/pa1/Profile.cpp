...
void Profile::deletePlaylist(int playlistId) {
    ...
    while((curr -> data).getPlaylistId() != playlistId) curr = curr -> next;
...
LinkedList<Playlist *> Profile::getSharedPlaylists() {
    ...
    Node<Profile *> *user = following.getFirstNode(), *head = following.getFirstNode();

    do
    {
        Node<Playlist> *playlist = (user -> data -> playlists).getFirstNode(), *first = (user -> data -> playlists).getFirstNode();

        do
        {
            if((playlist -> data).isShared()) shared.insertAtTheEnd(&(playlist -> data));

            playlist = playlist -> next;
        }while(playlist != first);
