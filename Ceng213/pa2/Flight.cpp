...
bool Flight::addTicket(const Ticket &ticket) {
    /* TODO */
    if(ticket.getTicketType() == economy && this -> economyCapacity > 0)
    {
        tickets.push_back(ticket);
        return true;
    }

    else if(ticket.getTicketType() == business && this -> businessCapacity > 0)
    {
        tickets.push_back(ticket);
        return true;
    }
...
bool Flight::operator<(const Flight &rhs) const {
    /* TODO */
    return this -> flightCode < rhs.flightCode;
}
